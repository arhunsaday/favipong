...
class PongGame {
  ...
  endGame() {
    this.gameState.gameActive = false;
    const winner =
      this.gameState.score1 >= 5 ? this.getPlayer1Id() : this.getPlayer2Id();
  ...
  movePaddle(playerId, direction) {
    ...
    if (player.isPlayer1) {
      this.gameState.paddle1.y = Math.max(
        0,
        Math.min(12, this.gameState.paddle1.y + direction)
      );
    } else {
      this.gameState.paddle2.y = Math.max(
        0,
        Math.min(12, this.gameState.paddle2.y + direction)
      );
    }
